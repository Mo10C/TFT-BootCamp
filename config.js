...
window.MCC_LB_CONFIG = {
  ...
  roles: {
    ...
    adminRoleIds: [],          // このロール保持者も管理者として扱う

    /* ★ 運営（観戦）ロール。
       ここに入れたロールを持つ人は「見るだけの人」になります。
         ・すべての画面を閲覧できる（ロックされない）
         ・大会の参加者・組卓・全体順位に入らない
         ・メンバー一覧・LPランキングに出ない
       大会に出てもらいたいときは、管理コンソールの「👥 メンバー」から
       その人の「大会に参加」をONにしてください。 */
    staffRoleIds: ["1540716169181397013"],   // ★ @運営ロール

    /* LPランキングの並び順（生徒 → 先生 → 校長・副校長 …）は
       ここではなく【管理コンソール →「📈 LP」タブ → 並び順】で設定します。
       Discordのロールを読み込んで、ドラッグで並べ替えるだけです。
       設定は Firestore に保存されるので、このファイルを触る必要はありません。

       ※ 下の lpGroups は旧バージョン（v3.7）の書き方です。
         管理コンソールで一度でも保存すると、そちらが優先されます。
         使っていなければ、この3行はそのままで構いません。 */
    lpGroups: []
  }
};
